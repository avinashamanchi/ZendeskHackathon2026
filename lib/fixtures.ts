...
// Relative timestamps resolve to real dates at module load so the demo never
// goes stale. d(6) = six days ago; the optional hours nudge keeps sibling
// events ordered and realistic (two charges twelve minutes apart, etc.).
const now = Date.now();
 ...
 */
export const maria: AccountState = {
...
/**
 * clean@example.com — the falsifiable claim. A healthy account: an old order,
 * delivered on time, one settled charge, nothing recent. MUST yield ZERO
 * hypotheses. If it doesn't, the rules are too loose and the thesis collapses.
 */
export const clean: AccountState = {
...
export const DEMO_CYCLE = [maria.email, sam.email, jo.email];

export function fixtureFor(email: string): AccountState {
  return FIXTURES[email.toLowerCase().trim()] ?? maria;
}
